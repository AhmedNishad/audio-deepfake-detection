...
def f_inference_single_samplev2(pt_model, device, data_in, checkpoint, data_info=None, data_tar=None, args=None):
    """ Wrapper for inference on a single sample
    
    Args:
        pt_model (torch.nn.Module): Pre-trained model.
        device (torch.device): Device to run the inference on (cpu or cuda).
        data_in (torch.Tensor or list): Input data tensor or list of tensors.
        data_info (str or None): Information about the input data (optional).
        data_tar (torch.Tensor or list or None): Target data tensor or list of tensors (optional).
        args (argparse.Namespace or None): Additional arguments (optional).
    
    Returns:
        torch.Tensor: Output generated by the model.
    """
    ...
    if isinstance(data_in, torch.Tensor):
        ...
        data_in = [x.to(device) for x in data_in]
    else:
        raise ValueError("data_in is not a tensor or list of tensors")

    if data_tar is not None:
        if isinstance(data_tar, torch.Tensor):
            data_tar = data_tar.to(device)
        elif isinstance(data_tar, list) and data_tar:
            data_tar = [x.to(device) for x in data_tar]
        else:
            raise ValueError("data_tar is not a tensor or list of tensors")
    ...
    with torch.no_grad():
        ...
        if data_tar is not None:
            output = infer_func(data_in, data_tar, data_info) if data_info else infer_func(data_in, data_tar)
        else:
            output = infer_func(data_in, data_info) if data_info else infer_func(data_in)
    
    return output


def f_inference_single_sample(args, pt_model, device, 
    ...
    with torch.no_grad():
        ...
        # run generation
        # Ensure there is at least one data sample in the loader
        if len(test_data_loader) > 0:
            # Get the first data sample
            data_idx, (data_in, data_tar, data_info, idx_orig) = next(enumerate(test_data_loader))

            # Decide whether to process this data sample or not
            if data_idx >= range_genidx_start and data_idx < range_genidx_end:
                ...
                if inf_datalist is not None:
                    ...
                    if seqname.seq_tag() not in inf_datalist:
                ...
                if isinstance(data_tar, torch.Tensor):
                    ...
                    data_tar = [x.to(device, dtype=nii_dconf.d_dtype) for x in data_tar]
                else:
                    pass
                ...
                if hasattr(pt_model, "inference"):
                    infer_func = pt_model.inference
                    nii_display.f_print("Mode is INFERENCE")
                else:
                    infer_func = pt_model.forward
                    nii_display.f_print("Mode is FORWARD")
                
                # Generate the output sequence
                if hasattr(args, 'trunc_input_length_for_inference') and args.trunc_input_length_for_inference > 0:
                    # Generate the data segment by segment, then do overlap and add
                    in_list, tar_list, overlap = nii_nn_tools.f_split_data(
                        data_in, data_tar, args.trunc_input_length_for_inference, args.trunc_input_overlap)
                    ...
                    for in_tmp, tar_tmp in zip(in_list, tar_list):
                        ...
                        if args.model_forward_with_target:
                            if args.model_forward_with_file_name:
                                ...
                                nii_display.f_print("Generating forward target 1 - truncate")
                            else:
                                data_gen = infer_func(in_tmp, tar_tmp)
                                nii_display.f_print("Generating forward target 2 - truncate")
                        else:    
                            if args.model_forward_with_file_name:
                                data_gen = infer_func(in_tmp, data_info)
                                nii_display.f_print("Generating file name 1 - truncate")
                            else:
                                data_gen = infer_func(in_tmp)
                                nii_display.f_print("Generating file name 2 - truncate")
                    ...
                    if len(gen_list) > 0 and len(in_list) > 0:
                        ...
                        data_gen = nii_nn_tools.f_overlap_data(gen_list, upsamp_fac * overlap)
                    else:
                        print("Generation failed on {:s}".format(data_info))
                        sys.exit(1)
                    ...
                    if args.model_forward_with_target:
                        ...
                        if args.model_forward_with_file_name:
                            ...
                            nii_display.f_print("Generating forward target 1 - whole")
                        else:
                            data_gen = infer_func(data_in, data_tar)
                            nii_display.f_print("Generating forward target 2 - whole")
                    else:    
                        if args.model_forward_with_file_name:
                            nii_display.f_print("{:s}".format(str(infer_func)))
                            data_gen = infer_func(data_in, data_info)
                            nii_display.f_print("Generating file name 1 - whole {:s}".format(str(data_gen)))
                        else:
                            data_gen = infer_func(data_in)
                            nii_display.f_print("Generating file name 2 - whole")
                
                # Log time for debugging
                nii_display.f_print(str(data_gen))
                ...
                if data_gen is not None:
                    try:
                        ...
                        print(data_gen_np)
                        print(data_gen)
                        nii_display.f_print(data_gen_np)
                        nii_display.f_print(data_gen)
                        return data_gen_np  # Return the output
                    except AttributeError:
                        mes = "Output data is not torch.tensor. Please check model.forward or model.inference"
                        nii_display.f_die(mes)

            # If the loop ends without returning, print the message and continue
