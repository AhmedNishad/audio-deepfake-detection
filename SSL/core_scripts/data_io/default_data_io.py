...
# Define the load_data_wrapper function outside of the class
def load_data_wrapper(self, x, y):
    return _data_reader(x, y, self.m_flag_lang, self.m_g2p_tool)

# Define the write_data_wrapper function outside of the class
def write_data_wrapper(self, x, y):
...
class NIIDataSet(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        # def load_data_wrapper(x, y):
        #     return _data_reader(x, y, self.m_flag_lang, self.m_g2p_tool)
        
        # def write_data_wrapper(x, y):
        #     return _data_writer(x, y, self.m_wav_sr)

        # method to load/write raw data
        if data_format == nii_dconf.h_dtype_str:
            self.f_load_data = partial(load_data_wrapper, self)
            self.f_length_data = _data_len_reader
            self.f_write_data = partial(write_data_wrapper, self)
        else:
